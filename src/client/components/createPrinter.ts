...
const createPrinter = (scene: THREE.Scene, camera: THREE.PerspectiveCamera, fontLoader:FontLoader) => {
    ...
    body_bottom_top.name = 'printer'
    body_bottom_top.position.set(-2, -33, 1)
    body_bottom_top.rotateX(Math.PI * 0.15)
    //body_bottom_top.rotateY(Math.PI * 0.1)
    //body_bottom_top.rotateZ(Math.PI * 0.1)
    ...
    const bio = new createText(camera,scene,fontLoader, 'Roboto Slab_Regular', "Teste", 0.05, 0.001, 'white', 0,0.13, 0, 0 ,true,body_bottom_top,-Math.PI*0.5,0,0)
